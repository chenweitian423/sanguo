...
- 燃烧林清兵 + 竹林密 → Boss **沙摩柯**(冰)
- **破阵 5 波（119 阵眼）**：每波一「阵眼」；波1/2/3/5 跳下首兵为眼，波4 枪兵蹿出首冲为眼；时限约 18s（宽松）
- 时限内破眼进下一波；超时则记失败、清场仍打完 5 波；**无论成败都出吕布**
- `formation_cleared` 仅全破五眼为 true；**吕布**仍掉干将+和氏璧（`has_thunder` / `has_heshi` / `sword_thunder`）
- Boss 线：**沙摩柯** → 破阵 → **吕布**(火) → **陆逊** → **左慈**(火/飞行)
